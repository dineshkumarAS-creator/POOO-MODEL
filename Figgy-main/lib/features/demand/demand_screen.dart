...
class _DemandScreenState extends State<DemandScreen> {
  ...
  Future<void> _loadWorkerId() async {
    ...
    setState(() => _workerId = prefs.getString('worker_id') ?? 'GS-DEMO-WORKER');
  }
  ...
  void _tick() {
    ...
    if (_subStep >= _subSteps) { 
      _subStep = 0; 
      _routeIndex++; 
      _emitTelemetry(); // Emit telemetry point every time we move to a new route node
    }
  }

  void _emitTelemetry() {
    if (_workerId == null || currentRide == null) return;

    // Simulate battery drain
    _batteryLevel -= 0.01;
    if (_batteryLevel < 10) _batteryLevel = 100.0; // Reset for demo

    // Calculate heading (mocked based on route index)
    double heading = 0.0;
    if (_routeIndex < routeCoordinates.length - 1) {
      final p1 = routeCoordinates[_routeIndex];
      final p2 = routeCoordinates[_routeIndex + 1];
      heading = atan2(p2.longitude - p1.longitude, p2.latitude - p1.latitude) * 180 / pi;
    }
    ...
    final payload = {
      ...
      'altitude': 12.0 + (Random().nextDouble() * 2.0), // Mock altitude
      'accuracy_m': 5.0 + (Random().nextDouble() * 5.0),
      'speed_kmh': currentRide!.status == 'on_the_way' ? (15 + Random().nextInt(25)).toDouble() : 0.0,
      'heading_degrees': heading,
      'app_foreground': true,
      'battery_percent': _batteryLevel.toInt(),
      'network_type': Random().nextBool() ? '5g' : '4g',
      'event_type': 'moving',
    };

    _apiService.submitTelemetry(payload);
  }
