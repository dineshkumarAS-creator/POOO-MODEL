  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(widget.isAppeal ? 'Attach Evidence' : 'Add proof', style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        centerTitle: true,
      ),
      body: Center(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Text('Step 2 of 3 — ${widget.isAppeal ? "Evidence" : "Add proof"}', style: const TextStyle(color: Colors.grey, fontSize: 12)),
                  ],
                ),
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(widget.isAppeal ? 'Appeal for' : 'Your claim', style: const TextStyle(color: Colors.grey, fontSize: 12)),
                            const SizedBox(height: 4),
                            const Text('Heavy Rain · ₹300 loss', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                          ],
                        ),
                      ),
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: widget.isAppeal ? Colors.orange.shade50 : Colors.green.shade50,
                          borderRadius: BorderRadius.circular(20),
                        ),
                        child: Text(
                          widget.isAppeal ? 'Formal Appeal' : 'Auto-triggered', 
                          style: TextStyle(color: widget.isAppeal ? Colors.orange.shade700 : Colors.green.shade700, fontSize: 10, fontWeight: FontWeight.bold)
                        ),
                      ),
                    ],
                  ),
                ...
                const SizedBox(height: 32),
                Text(widget.isAppeal ? 'Critical Evidence' : 'What proof helps', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                        ...
                        children: [
                          Text(
                            _uploadedFiles.length >= 3 
                              ? 'Max files reached' 
                              : (widget.isAppeal ? 'Upload up to 3 evidence files' : 'Up to 3 photos or files'), 
                        ...
                        Expanded(
                          child: RichText(
                            text: TextSpan(
                              ...
                              children: [
                                TextSpan(text: widget.isAppeal ? 'Evidence added — review chance: ' : 'Proof added — approval chance: '),
                                TextSpan(
                ...
                ElevatedButton(
                  onPressed: _uploadedFiles.isEmpty ? null : () {
                    // In a production app, we would upload to S3/Firebase here.
                    // For the demo, we pass the filenames as simulated URLs.
                    final List<String> mockUrls = _uploadedFiles.map((f) => "https://storage.figgy.in/proofs/${f.name}").toList();
                    
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => ReviewClaimScreen(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(widget.isAppeal ? 'Final Review' : 'Continue to review', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                      const SizedBox(width: 12),
