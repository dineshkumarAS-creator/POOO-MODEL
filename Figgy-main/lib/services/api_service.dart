...
class ApiService {
  ...
  /// Disputes a rejected claim by providing a worker statement and optional evidence.
  Future<Map<String, dynamic>> appealClaim(
    String claimId, {
    ...
    try {
      ...
      final body = json.decode(response.body) as Map<String, dynamic>;
      if (response.statusCode == 201) return body;
      throw ApiException(body['message'] ?? 'Failed to file appeal', statusCode: response.statusCode);
    } catch (e) {
  ...
  /// Sends a raw telemetry point to the backend store.
  Future<void> submitTelemetry(Map<String, dynamic> data) async {
    final uri = Uri.parse('$_baseUrl/api/worker/telemetry');
    try {
      await http.post(
        uri,
        headers: _jsonHeaders,
      ...
      // In a real app we'd buffer these, but for a hackathon we just log errors.
      debugPrint('[ApiService.submitTelemetry] Error: $e');
    }
  }
}
