...
def appeal_claim(claim_id: str):
    ...
    try:
        ...
        # Support both single and multiple proof URLs
        proof_urls = data.get("proof_urls", [])
        if not proof_urls and data.get("proof_url"):
            proof_urls = [data.get("proof_url")]
        ...
        rejection_reason = claim.get("rejection_reason", "").upper()
        # Expanded allowed reasons: includes GPS deviation often found in manual/auto mocks
        allowed_reasons = ["NO_DISRUPTION_DETECTED", "OUTSIDE_COVERAGE_ZONE", "GPS ERROR", "GPS_ERROR"]
        if rejection_reason not in allowed_reasons:
            return jsonify({
                "status": "error", 
                "message": f"This claim '{rejection_reason}' is not eligible for automated appeal. Contact specialist."
            }), 400
        ...
        appeal_claim_id = _generate_claim_id()
        ...
        appeal_doc = {
            **claim,
            ...
            "proof_urls": proof_urls if proof_urls else claim.get("proof_urls", []),
            "created_at": _now_iso(),
            "updated_at": _now_iso(),
            "resolved_at": None,
        ...
        return jsonify({
            ...
            "message": "Appeal received. Specialist review within 24 hours."
        }), 201
